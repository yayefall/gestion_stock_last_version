    ...
    <q-card>
     ...
     <q-dialog v-model="isDialogOpens">
        <q-card style="width: 400px;">
          <q-card-section>
            <div class="text-h6">Rapport Versement</div>
          </q-card-section>
        ...
        <template v-slot:body-cell-actions="props">
          <q-td :props="props">
            <q-btn
              icon="delete"
              color="negative"
              @click="deleteVersement(props.row.id)"
            />
          </q-td>
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { name: "client_nom", label: "Client", align: "left", field: "client_nom",},
  ...
  methods: {
    ...
    async fetchVersements() {
  try {
    // Appel API pour récupérer les versements
    const response = await axios.get("http://localhost:4000/api/versement");
    ...
    async submitVersement() {
      try {
        await axios.post("http://localhost:4000/api/versement", this.versement);
        this.$q.notify({
          type: "positive",
          message: "Versement ajouté avec succès.",
        });
        this.isDialogOpen = false; // Fermer le dialog
        this.fetchVersements(); // Recharger la liste des versements
        this.resetForm();
      } catch (error) {
    ...
    filterVersements() {
      ...
      this.filteredVersements = this.versements.filter(
        (v) =>
          ...
          v.client_id.toString().includes(term) ||
  ...
  async deleteVersement(id) {
    try {
      ...
      if (confirmed) {
        await axios.delete(`http://localhost:4000/api/versement/${id}`);
        this.$q.notify({
          type: "positive",
