...
export default {
  data() {
    return {
      ...
      dialogTitle: 'Ajouter un Mouvement ',
  ...
  exportTable() {
    ...
    // fonction pour exporter les produits en pdf
     exportToPDF() {
       ...
       // console.log('Lignes pour le tableau :', rows);
        ...
        // Ajout du titre
        doc.text('Liste des Mouvements de stocks', 14, 10);
    ...
    /*  openDialog() {
      this.dialogTitle = 'Ajouter un Mouvement';
      this.mouvement = {
    ...
    editMouvement(mouvements) {
      this.dialogTitle = 'Modifier le Mouvement';
    ...
    async saveStockMovement() {
      try {
        ...
        const payload = {
          produit_id: this.mouvement.produit_id.id,
        ...
        if (this.mouvement.id) {
          // Modification
          response = await axios.put(`http://localhost:4000/api/mouvements_stock/${this.mouvement.id}`, payload);
          this.$q.notify({
          ...
          response = await axios.post("http://localhost:4000/api/mouvements_stock", payload);
          this.$q.notify({
   ...
   async editMouvement(mouvement) {
      this.dialog = true;
      this.mouvement = {
        id: mouvement.id,
        produit_id: this.produitsOptions.find(p => p.id === mouvement.produit_id),
        date: mouvement.date,
        type: mouvement.type,
    ...
    // fonction pour supprimer un mouvement de stock
